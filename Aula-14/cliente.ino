...
void printPacote(byte *pac, int tamanho){
      ...
      Serial.print(F(" I: "));
      Serial.print(pac[4]);  // print the payload's value
      Serial.print(F(" : "));
...
void loop() {
  ...
  if (Serial.available()) {
    ...
    if (c == 'T') {
      ...
      bool report = sendPacket(&payload[0], sizeof(payload), destino, RTS);  // transmit & save the report
      report = aguardaMsg(CTS);
      if(report){
        sendPacket(&payload[0], sizeof(payload), destino, MSG);
